...
typedef NS_ENUM(NSUInteger, TVOIceCandidatePairState) {
    TVOIceCandidatePairStateSucceeded,      ///< Ice candidate pair state Succeeded
    TVOIceCandidatePairStateFrozen,         ///< Ice candidate pair state Frozen
    TVOIceCandidatePairStateWaiting,        ///< Ice candidate pair state Waiting
    TVOIceCandidatePairStateInProgress,     ///< Ice candidate pair state in progress
    TVOIceCandidatePairStateFailed,         ///< Ice candidate pair state state Failed
    TVOIceCandidatePairStateCancelled,      ///< Ice candidate pair state state Cancelled
    TVOIceCandidatePairStateUnknown         ///< Unknown Ice candidate pair state
}
...
/**
 * @brief The unique identifier used for TVOIceCandidateStats associated with this candidate.
 */
@property (nonatomic, copy, readonly, nullable) NSString *transportId;

/**
 * @brief The unique identifier that is associated to the object that was inspected to produce the Ice candidate
 * attributes for the local candidate associated with this candidate pair.
 */
...
/**
 * @brief The unique identifier that is associated to the object that was inspected to produce the Ice candidate
 * attributes for the remote candidate associated with this candidate pair.
 */
...
/**
 * @brief Represents the total number of payload bytes sent on this candidate pair, i.e., not including headers or
 * padding.
 */
@property (nonatomic, assign, readonly) uint64_t bytesSent;

/**
 * @brief Represents the total number of payload bytes received on this candidate pair, i.e., not including headers or
 * padding.
 */
 ...
 *
 * @discussion Represents the sum of all round trip time measurements in seconds since the beginning of the session,
 * based on both STUN connectivity check [ STUN-PATH-CHAR] responses (responsesReceived) and consent [ RFC7675]
 * responses (consentResponsesReceived). The average round trip time can be computed from totalRoundTripTime by
 * dividing it by (responsesReceived + consentResponsesReceived).
 */
 ...
 *
 * @discussion Represents the latest round trip time measured in seconds, computed from both STUN connectivity checks
 * [STUN-PATH-CHAR] and consent responses [RFC7675].
 */
 ...
 *
 * @discussion It is calculated by the underlying congestion control by combining the available bitrate for all the
 * outgoing RTP streams using this candidate pair. The bitrate measurement does not count the size of the IP or other
 * transport layers like TCP or UDP. It is similar to the TIAS defined in [RFC3890], i.e., it is measured in bits per
 ...
 *
 * @discussion It is calculated by the underlying congestion control by combining the available bitrate for all the
 * incoming RTP streams using this candidate pair. The bitrate measurement does not count the size of the IP or other
 * transport layers like TCP or UDP. It is similar to the TIAS defined in [RFC3890], i.e., it is measured in bits per
 * second and the bitrate is calculated over a 1 second window.
 */
 ...
 */
- (null_unspecified instancetype)init __attribute__((unavailable("TVOIceCandidatePairStats cannot be created explicitly.")));
