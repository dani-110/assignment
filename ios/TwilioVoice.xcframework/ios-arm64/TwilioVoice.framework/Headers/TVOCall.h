...
@protocol TVOCallMessageDelegate;
 ...
 *
 * @discussion The call needs to be in the `<state>` `TVOCallStateConnected` for stats reports to be delivered.
 *
...
/**
 * @brief Sends a user-defined message to the endpoints that subscribe to the user-defined messages of this call.
 *
 * @param callMessage The `TVOCallMessage` object with the message and configuration.
 *
 * @discussion The result will be raised to the `TVOCallMessageDelegate` provided in the `TVOCallOptions` on the same
 * dispatch queue used for connecting the call. Dispatch main queue will be used if not specified.
 *
 * @discussion Sending a call message with content size that exceeds 10 KB or sending more than 10 call messages
 ...
 *
 * @discussion A subscription with a user-defined message callback is required prior to calling this method. Calling the
 * `[TVOCall sendMessage:]` method without subscribing first will result in an error in your Twilio developer console.
 *
 * @discussion This feature is currently in Beta.
 *
 * @return A unique identifier for tracking the message.
 */
- (nonnull NSString *)sendMessage:(nonnull TVOCallMessage *)callMessage
NS_SWIFT_NAME(sendMessage(_:));
